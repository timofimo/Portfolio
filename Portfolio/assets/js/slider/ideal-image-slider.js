...
var IdealImageSlider = (function() {
	...
	var _loadImg = function(slide, callback) {
		if (!slide.style.backgroundImage) {
			...
			img.onload = function() {
				var address = "url('";
				address += slide.getAttribute('data-src');
				address += "')";
				slide.style.backgroundImage = address;
	...
	var Slider = function(args) {
		...
		this.settings = {
			selector: '',
			position: 'left', // for width percentage - "left" | "center" | "right"
			width: 'auto', // "auto" | px value (e.g. 400) | percentage (e.g. "80%")
		...
		// Set width
		if (_isInteger(this.settings.width)) {
			this._attributes.container.style.width = this.settings.width + 'px';
		} else {
			// If percentage
			if (this.settings.width.indexOf('%') > -1) {
				this._attributes.container.style.width = this.settings.width;

				var percentage = this.settings.width.split('%');
				if(this.settings.position === 'center'){
					this._attributes.container.style.marginLeft = (100.0 - parseFloat(percentage[0])) * 0.5 + '%';
				} else if(this.settings.position === 'right'){
					this._attributes.container.style.marginLeft = (100.0 - parseFloat(percentage[0])) + '%';
				}
			}
		}
